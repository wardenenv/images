...
BUILD_DIR="$(dirname ${SEARCH_PATH}/${VARIANT})"
...
if [[ ${IMAGE_SUFFIX} =~ xdebug3 ]] && test $(version ${PHP_VERSION}) -lt $(version "7.2"); then
  echo "::notice file=php-fpm/scripts/build.sh,line=96,title=::Skipping build for ${IMAGE_NAME}:${IMAGE_TAG} (xdebug3 is unavailable for PHP/${PHP_VERSION})"
  exit 0
fi
...
#docker buildx build --load -t "${IMAGE_NAME}:build" $(printf -- "--build-arg %s " "${BUILD_ARGS[@]}") "${BUILD_CONTEXT}" 
